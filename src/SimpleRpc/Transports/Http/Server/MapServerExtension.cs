...
namespace SimpleRpc.Transports.Http.Server
{
    public static class MapServerExtension
    {
        ...
        {
            return app.MapPost(pattern, async Task<Results<FileContentHttpResult, FileStreamHttpResult, BadRequest<string>>> (RpcRequest request, RpcServer<TService> rpcServer) =>
            {
